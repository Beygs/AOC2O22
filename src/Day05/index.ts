...
const formatSchema = (data: string): string[][] => {
...
const Day05 = () => {
  ...
  let newSchema: string[][] = JSON.parse(JSON.stringify(schema));
  ...
  let newSchema2: string[][] = JSON.parse(JSON.stringify(schema));

  indications.forEach((indication, id) => {
    if (id === indications.length) return;
